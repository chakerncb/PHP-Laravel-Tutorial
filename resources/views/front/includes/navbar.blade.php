<nav class="navbar navbar-expand-lg navbar-light bg-white py-3">
    <div class="container px-5">
        <a class="navbar-brand" href="{{route('index')}}"><span
                class="fw-bolder text-primary">{{ config('app.name') }}</span></a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <div class="d-flex gap-2" style="margin-left: auto;">
                @guest
                    @if (Route::has('login'))
                        <a class="btn btn-outline-dark btn-lg px-3 py-1 fs-6 fw-bolder"
                            href="{{ route('login') }}">{{ __('Login') }}</a>
                    @endif

                    @if (Route::has('register'))
                        <a class="btn btn-primary btn-lg px-2 py-1 fs-6 fw-bolder"
